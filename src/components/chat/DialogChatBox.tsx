...
export const DialogChatBox = ({ brand, insights }: ChatBoxProps) => {
  ...
  return (
    <div className={`flex flex-col bg-white/95 rounded-lg p-4 border border-[#3E66FB]/50 transition-all duration-300 ease-in-out ${isExpanded ? 'h-[300px]' : 'h-[120px]'}`}>
      <h3 className="text-lg font-semibold text-gray-900 mb-2">Ask about the insights</h3>
      
      <div className="flex-1 overflow-hidden flex flex-col min-h-0">
        {messages.length > 0 && (
          <ScrollArea className="flex-1 pr-4 mb-4">
            <div className="space-y-4 text-sm sm:text-base pb-2">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row gap-2 mt-auto">
          <Input
            ...
            disabled={isLoading}
            className="flex-1 w-full text-sm sm:text-base text-gray-900 bg-white border-[#3E66FB]/20"
          />
          <Button 
            type="submit" 
            disabled={isLoading} 
